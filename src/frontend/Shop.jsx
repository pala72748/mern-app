                                ...
                                <div className="lg:col-span-3">
                                    <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                                        {viewblogdata && viewblogdata.map((product, index) => (
                                            <div key={index} className="group">
                                                ...
                                                <p className="mt-1 text-lg font-medium text-gray-900"> <div dangerouslySetInnerHTML={{ __html: product.product_desc.substring(0, 100) }}></div></p>
                                            </div>
                                        ))}
                                    </div>
                                    <div className="flex justify-center">
                                        <div className="inline-flex items-center justify-center rounded-md border-[0.8px] border-gray-3 shadow-1 mt-17.5">
                                            {activePage !== 1 &&
                                                <button onClick={() => setActivePage(activePage - 1)} href="javascript:void(null)" className="page-link rounded-l-md text-[#5A6A85] py-1 lsm:py-2.5 px-3 lsm:px-4.5 border-r border-gray-3 ease-in duration-300 hover:text-dark hover:bg-gray">
                                                    Previous
                                                </button>
                                            }

                                            {totalPages(totalUser, limit).map(page => (
                                                <button key={page} className="text-[#5A6A85] py-1 lsm:py-2.5 px-3 lsm:px-4.5 border-r border-gray-3 ease-in duration-300 hover:text-dark hover:bg-gray">
                                                    {page}
                                                </button>
                                            ))}
                                            {activePage !== totalPages(totalUser, limit).length &&
                                                <button onClick={() => setActivePage(activePage + 1)} href="javascript:void(null)" className="page-link rounded-r-md text-[#5A6A85] py-1 lsm:py-2.5 px-3 lsm:px-4.5 border-r border-gray-3 ease-in duration-300 hover:text-dark hover:bg-gray">
                                                    Next
                                                </button>
                                            }
                                        </div>
